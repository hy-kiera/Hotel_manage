...
public class ExamplePlayerBackActivity extends Activity {

	private static final String APPLICATION_ID = "JxaJcSz9glaCUMAm7o6Rhw";
	private static final String API_KEY = "61w9lrhcxap2q62s91vmsptrp";
	...
	public class TextViewPagerAdapter extends PagerAdapter {
		...
		public Object instantiateItem(ViewGroup container, int position) {
			...
			if (mContext != null) {
				...
				view = inflater.inflate(R.layout.activity_example_player, container, false);
...
//				textView.setText("TEXT " + position) ;
	...
	private void init2(){
		...
		((Button)findViewById(R.id.PlayNext)).setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View v) {
				int size = mPlayList.size();

				if(mPlayList != null && size > 0){
					if(playcount < size) {
						((TextView)findViewById(R.id.PlaylistCount)).setText("now : " + playcount + "size : " + size);
						initPlayer(mPlayList.get(playcount),"");
						playcount++;
					}else{
						playcount = 0;
						Toast.makeText(context, "initPlayer LAST",Toast.LENGTH_LONG).show();
						Log.d(TAG,"initPlayer LAST");
					}
				}
			}
		});
		...
		public void onStateChange(PlayerState state) {
			...
			if (state == PlayerState.PLAYING || state == PlayerState.PAUSED || state == PlayerState.COMPLETED) {
				if (mMediaController == null && mBroadcastPlayer != null && !mBroadcastPlayer.isTypeLive()) {
					mMediaController = new MediaController(ExamplePlayerBackActivity.this);
